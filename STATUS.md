...
| 頁面內容（18 頁動態表單） | ✅ 可運作 —— 表單由 `GET /admin/page-schema/{key}` 動態生成<br>已涵蓋 26 個區段、168 個欄位，10 種 `x-fieldType` 全支援；缺 RTE（暫用純文字區）|
| 產品列表 | ✅ 可運作（搜尋／狀態篩選／分頁／完整度儀表）|
| 產品編輯 | 🟡 可運作（雙語分頁、三個 repeater、圖庫＋主圖、使用情境照、部位／認證多選、尺寸表編輯器、發布／取消發布、`rowVersion` 併發、移除語系）<br>缺：相關產品拖曳、RTE（TipTap 待 code-split）|
...
| 應用方案編輯（含人體圖座標選取器） | 🟡 可運作（兩種型態、雙語、三張圖、四組 repeater、推薦產品、發布／取消發布、移除語系、刪除）<br>人體圖選取器：點放／拖曳／方向鍵微調、顯示其他方案的既有座標避免疊圖<br>缺：RTE（內文暫用純文字區）|
...
| 6 | **149 筆產品的 zh-TW 翻譯**（匯入的是英文舊站資料） | 整個中文站的產品面：產品列表 0 筆、分類頁空格線、首頁精選與應用方案的推薦產品皆為空。<br>這是**目前最大的內容缺口**，不是程式問題 —— 語言純度按規格運作中 |
