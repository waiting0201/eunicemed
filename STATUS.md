...
- **密碼下限 12 → 6，且不再強制首次登入改密碼。** `MustChangePassword` 的三個寫入點
  （建立帳號、管理者重設密碼、種子帳號）全部拿掉，DTO 與畫面上的「待改密碼」一併移除；
  欄位本身留在資料庫（永遠 `false`），為了一個沒人讀的 bit 做破壞性遷移不划算。
  改密碼撤銷所有 session 這一項不變。
- **圖片／PDF 改成按下儲存才上傳。** 選檔案只做本機預覽（`URL.createObjectURL`）
  與當場量尺寸的警示，`staged:` 暫時 id 在存檔時由 `commitStagedMedia()` 換成真正的
  `mediaId`。先前選完就傳，按取消或改選一張都會在 Blob 留下清不掉的孤兒圖
  （後台沒有媒體庫畫面）。見 [docs/03](docs/03-cms.md) §6。
...
| 媒體欄位（無獨立畫面）| ✅ 每個圖片／檔案欄位就地選本機檔：尺寸與比例提示、**選檔即預覽**、非阻擋警告、alt 輸入格、圖庫可多選、**PDF 直傳＋登記**<br>**2026-09-16 起按下儲存才上傳**（`lib/mediaStaging.ts`）—— 沒存檔就不會在雲端留下東西<br>刻意放棄：引用反查、刪除媒體、reprocess |
| ~~導覽選單／轉址／設定~~ | **已移除**（2026-08-29，見 [docs/15](docs/15-cms-scope.md) §7.4／§7.5）|
| 使用者 | ✅ 可運作（角色、停用、重設密碼、解鎖、刪除）。密碼至少 6 碼，**不強制首次改密**（2026-09-16）|
