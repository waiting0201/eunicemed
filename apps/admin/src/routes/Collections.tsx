...
function CollectionDialog({
  ...
  const save = useMutation({
    mutationFn: () =>
      api.saveCollection(collection.id, { sortOrder, strength: collection.strength, translations }),
    onSuccess: onSaved,
    onError: (e) => setError(e instanceof ApiError ? e.message : '儲存失敗。'),
  });

  const patch = (locale: Locale, p: Partial<{ name: string; description: string }>) =>
    setTranslations((t) => ({
      ...t,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-6">
      <div className="panel w-[min(36rem,92vw)] max-h-[88vh] overflow-y-auto">
