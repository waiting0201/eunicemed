...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  return pageMetadata({
    ...
    title: c.metaTitle,
    description: c.lead,
    // 這一頁的內容整份寫在程式碼裡（沒有 api.page），兩個語系必定都在，
    // 但後端的 sitemap 還沒收錄 contact（`SiteHandler.StaticPaths`）——
    // 交給 hreflang 查表會查不到而只印自己，所以在這裡直接指定。
    locales: [...LOCALES],
  });
}
...
export default async function ContactPage({ params }: { params: Promise<Params> }) {
  ...
  return (
    <>
      <JsonLd data={contactPageSchema(locale, '/contact')} />
