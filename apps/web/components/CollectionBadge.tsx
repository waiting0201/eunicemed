...
import { collectionBg } from '@/lib/collection';
...
export function CollectionBadge({ collection }: { collection: SlugName }) {
  return (
    <span
      className={`inline-block rounded-full px-3.5 py-1 text-[0.7rem] font-bold uppercase tracking-[0.1em] text-white ${collectionBg(
        collection.slug,
      )}`}
    >
      {collection.name}
