...
SVG 清洗與 `MediaUsage` 自動重建已於 Phase 5 補上；
`reprocess` 與上傳去重於 2026-08-31 補上（見下方踩坑），Phase 3 至此完整。
...
檔名帶的是**內容雜湊**，所以同一個檔案上傳兩次會寫回同一組 blob，而
`POST /admin/media` 不去重，於是多一列 `Media` 指著別人的檔案。
`DELETE /admin/media/{id}` 的 409 只檢查**被刪的那一列**有沒有引用，
通過之後就把 blob 刪掉 —— 另一列即使正被頁面引用，當場變成 404 的死連結，
畫面上只看到破圖，沒有任何錯誤訊息。

同一個雜湊還有第二個問題：master 檔名不含 preset，但**階梯與 master 寬度是照
preset 走的**。同一張圖先傳 `photo-4x3` 再傳 `card-16x10`，第二次會把第一次的
master 覆寫成 800px，而第一列的 `Width` 還寫著 1200 —— 資料與檔案就此對不起來。

三處一起修：

1. **雜湊改算「內容 + presetKey」**（`ImageService.Render`）。檔名從此唯一標識
   「這份內容 + 這個 preset」，跨 preset 不再互相覆寫。
2. **上傳以檔名去重**：命中既有那筆就直接回它（200，不是 201），連 blob 都不重寫。
   PDF 的 `POST /admin/uploads/register` 本來就這樣做，這次讓點陣與 SVG 對齊。
3. **刪除時只刪沒有別列共用的 blob**。去重擋的是**未來**的重複列；
   修正之前上傳的重複列還在庫裡，那些才是會踩到的。

驗證方式是照著正式庫的形狀，手動插一列共用同一組 blob 的 `Media`（連 variant 列
一起複製），刪掉它之後確認另一列的 5 個檔案仍然 200。
