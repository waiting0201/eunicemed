...
> ℹ️ **語言純度的一個已知例外（2026-08-19）**：8 篇 Insights 衛教長文在 zh-TW 放的是**英文原文**。
> 系統沒有「回退到另一語言」的機制（公開查詢是 translation 的 INNER JOIN），
> 所以要讓中文站看得到，只能在 zh-TW 放一份內容。這是應要求的決定 ——
> 那幾篇每篇 2,000–3,700 字，客戶可能想重寫而非翻譯；在那之前，看得到英文版比看不到有用。
> 拿到中文稿後直接覆蓋 zh-TW 即可，不需要改程式。搬運腳本見 `tools/migration/insights-zh-fallback.py`。
