...
**改之後**：每個欄位一顆「選擇圖片」開系統選檔視窗，選完就地預覽並套用。
**檔案在按下儲存時才真的上傳**（2026-09-16）—— 沒有媒體庫可以清孤兒圖，
就不能讓「點開看看」在雲端留下東西。見 [03](03-cms.md) §6。
...
> ⚠️ **【2026-09-17 推翻，見 §10】** 上面這段的「維持寫死」已不成立：
> 10 頁的 band 一律由 CMS 供給，`BRAND_BANDS` 降為沒換圖時的預設值。
> 本節其餘內容（圖樣來源、裁切與階梯）不變。
...
## 10. 2026-09-17：那 6 頁的頁頂圖改回後台可換（客戶要求）

§9.1 把 Applications／FAQ／Insights／News／Downloads／Where to Buy 的 band
跟著版面文案一起寫死。**這一項推翻**：10 頁的 band 一律由 CMS 的 `hero.band` 供給。

推翻的理由就是 §9.1 自己留下的那句代價 ——「後台會出現有些頁能換頁頂圖、有些不能的
不一致，需要在交接時講一句」。實際交接時那句話講不通：**站在後台看，那十條 band
長得一模一樣**，「這四頁的是形象照版位、那六頁的是品牌圖樣」是我們的分類，不是
編輯者的。頁頂圖是換起來最沒有風險的東西（preset 鎖住 16:3，換圖撞不壞版型），
用 §2 的判準它本來就該進 CMS，§9.1 是被「這 6 頁沒有 schema 檔」的實作現況帶著走。

### 10.1 做法

- 補 6 支 `{pageKey}.hero.json`，內容與 `privacy.hero.json` 逐字相同（只有一個
  `band`，`x-mediaPreset: page-band`、`x-localeInvariant`）。**沒有 migration** ——
  `PageSection` 列由啟動時的同步器依 schema 目錄建立（本機實測：`新增 6、停用 0`）。
- `BRAND_BANDS` 從「寫死的圖」變成**預設圖**：`lib/bands.ts` 的 `pageBand(page, fallback)`
  取得出 `url` 就用編輯者的，否則退回原本那三張品牌圖樣。
  **這 6 頁沒有人會特地去設一張圖**，預設圖直接是對的那一張，不是一條空白。
- 那三張品牌圖樣**留在 `public/brand/bands/`**，不改成上傳一次進 Blob ——
  預設值進資料庫就會變成「可以被刪掉的預設值」。
- 退回預設的條件是**解析得出 `url`**，不是「欄位有值」。媒體被刪掉的引用由
  `SectionWalker.ResolveMedia` 整個移除（安全），但值若不是合法 uuid，後端不認得那是
  媒體欄位，會**原字串回傳** —— 直接往下傳會渲染出 `src={undefined}` 的破圖。

### 10.2 連帶

- `GET /admin/pages` 由 6 列變 **12 列**（判準沒改，仍是「有沒有啟用中的區段」）。
  濾掉的剩 6 頁：Contact 與四支共用版型。
- 清單底下那句 `form-hint` 改寫 —— 原句說這幾頁「版面文字與頁頂圖固定不變」，
  現在頁頂圖能改了。新的困惑點也換了位置：**這幾頁點進去只有一張圖**，
  條目本身在側欄各自的項目裡，這才是要講的那句。
- `Api/PageSchemas/` 19 支 → **25 支**，pageKey 6 → **12**。
