...
> ⚠️ **【2026-08-28 收斂，見 [15-cms-scope.md](15-cms-scope.md)；2026-09-17 補 6 支，見 §10】**
> 實際有 schema 的是 **12 個 pageKey / 25 個區段**，其餘 6 頁（Contact 與四支共用版型）
> 在這個畫面是空的 —— **那是定案不是壞掉**：那些頁的版面文案已寫死在前端。
> 留在區段裡的以圖片與引用清單為主（換圖不會撞壞版型，挑要露出哪幾份是策展）。
> 後補的那 6 支（Applications／FAQ／Insights／News／Downloads／Where to Buy）**只有頁頂圖**
> 一個欄位，沒設就顯示品牌圖樣預設圖。
