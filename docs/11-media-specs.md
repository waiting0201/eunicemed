...
### 2a. 變體階梯（每次上傳實際產生哪些檔案）

**這些是要產生的實體檔案，不是建議值。** 本站無 CDN、不使用 `next/image` 即時優化（[07](07-azure-deployment.md) §7.3），
Blob 只會把檔案原樣吐出來不會幫你縮 —— **沒在上傳當下產生的尺寸，之後就不存在**，只能靠 `reprocess` 補。

| presetKey | WebP 階梯 | 原格式 | 檔數 |
|---|---|---|---|
| `hero-slide` / `page-band` / `section-bg` | 2560 · 1600 · 1200 · 800 | 2560 | 5 |
| `wide-16x9` | 1600 · 1200 · 800 | 1600 | 4 |
...
機器可讀版本在 `Api/Media/media-presets.json` 的 `output` 欄位，**程式一律讀那份**。

**為什麼原格式只出一張**：WebP 的瀏覽器支援率已超過 97%（Safari 自 2020 年起支援），
原格式實質上只是退路，出全套只會白白拉長上傳時間。
**為什麼 `og-image` 不出 WebP**：社群爬蟲（Facebook、LINE 等）對 WebP 的支援不一致，OG 圖只出原格式最安全。

**這個決策的成本**（2026-08-17 定案）：
- 上傳延遲：2560px 的 WebP 編碼約 0.5–1.5 秒/張，`hero-slide` 一次上傳最多 5 個檔，後台需有進度狀態。
- **Function App 實例需 2048MB，512MB 不夠** —— 解碼 8000px 來源圖再編多個尺寸會爆記憶體。見 [07](07-azure-deployment.md) §10。
- 儲存空間約為單張的 3–5 倍（此項成本可忽略）。

> **背景**：原始規格是「輸出 2 個檔，響應式尺寸由 `next/image` 於請求時產生」——
> 那在有 CDN／可用 SWA 圖片優化的前提下是自洽的。
> 「不用 CDN、圖片走 Blob 直連」的部署決策把那條路封死，才必須改成預先產生階梯。

**比例不符時**：後台顯示警示（「此欄位建議 1:1，您的圖為 3:4，兩側會被裁切」）但**不阻擋**上傳；前端一律 `object-fit: cover` 置中裁切。**檔案過大**同樣是警示不阻擋（硬上限見 §4）。

> **警示在選檔的當下就出現**，不是等上傳回來 —— 檔案要按下儲存才會上傳（[03](03-cms.md) §6），
> 等伺服器回話等於整個填表過程都沒有回饋。所以 §4 的三條非阻擋規則
> （比例 5%、寬度 80%、建議大小）**在瀏覽器與伺服器各實作一次**：
> `apps/admin/src/lib/mediaStaging.ts` 的 `inspect()` 與 `Api/Services/ImageService.cs`
> 的 `Inspect()`。門檻要改就兩邊一起改。伺服器上傳後回的那一組仍會顯示
> （以真正的 `mediaId` 為 key），所以只有伺服器看得出來的差異不會被吃掉。
...
| `measure-diagram` | 1:2 | **400 × 800** | ≤ 80 KB | 產品尺寸表旁的量測部位線稿，直式人形（PNG 透明底或 SVG） |
...
| 項目 | 限制 | 違反時 |
|---|---|---|
| 圖片格式 | `jpg` `jpeg` `png` `webp`（`svg` 僅 `logo-mark`／`measure-diagram`） | 415 拒絕 |
| 文件格式 | `pdf` | 415 拒絕 |
| 原始檔大小 | 圖片 ≤ 20 MB；PDF ≤ 20 MB | 413 拒絕 |
| 原始圖片尺寸 | 長邊 ≤ 8000 px、總像素 ≤ 40 MP | 400 拒絕（防解壓縮炸彈） |
| 比例偏差 | 與 preset 相差 > 5% | **警示，不阻擋**（選檔當下即提示） |
